...
\begin{enumerate}
    \item A \textbf{consensus core} (claims that survived critique across stances),
    \item A set of \textbf{conditional or branch claims} (``if autonomy is prioritized over aggregate welfare, then\ldots''), and
    \item A \textbf{minority report / dissent memo} that records unresolved conflicts, the strongest arguments on each side, and which premises are contested.
\end{enumerate}
