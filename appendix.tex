...
\textbf{Pseudocode:}
\begin{verbatim}
function DIALECTICAL_REFINE(initial_proposal, context, perspective):
    proposal = initial_proposal
    history = []

    for i in 1 to MAX_ITERATIONS:
        # Generate criticism of current proposal
        criticism = BEST_OF_N(
            CRITIC(proposal, context, perspective), n=3)

        # Evaluate proposal vs criticism
        score_diff = EVALUATOR(proposal, criticism)
        history.append(score_diff)

        # Check convergence conditions
        if variance(history) < THRESHOLD:
            return proposal  # Converged
        if score_diff > 5.0:
            return proposal  # Strong defense

        # Refine proposal based on valid criticism
        proposal = BEST_OF_N(
            REFINER(proposal, criticism, context), n=3)
...
\textbf{Traversal Pseudocode:}
\begin{verbatim}
function TRAVERSE_TEMPLATE_TREE(root, perspective, plan, budget):
    traversals = []
    queue = [(root, [], budget)]

    while queue not empty:
        (node, path, remaining_budget) = queue.pop()
        path = path + [node]

        if node.type == "leaf":
            # Create traversal record for research job
            traversals.append(TemplateTraversal(
                path_nodes=path,
                word_budget=node.word_budget,
                syllogism_type=node.syllogism_type,
                research_order=len(traversals) + 1
            ))
        else:
            # Select children based on perspective/plan
            selected = LLM_SELECT(
                node.children,
                node.children_choice_prompt,
                perspective, plan)

            # Allocate budget to selected children
            allocations = ALLOCATE_BUDGET(
                selected, remaining_budget)

            for child, alloc in zip(selected, allocations):
                queue.append((child, path, alloc))
...
\textbf{Evidence Selection Pseudocode:}
\begin{verbatim}
function SELECT_EVIDENCE(source_document, argument_context):
    # Index all sentences in source
    sentences = {}
    for i, sent in enumerate(split_sentences(source_document)):
        sentences[f"sent_{i}"] = sent

    # LLM selects relevant sentence IDs (not text)
    selected_ids = LLM_SELECT_SENTENCES(
        sentence_index=sentences.keys(),
        context=argument_context,
        max_sentences=5)

    # Assemble evidence from original text
    evidence_text = ""
    for sid in selected_ids:
        evidence_text += sentences[sid] + " /.../ "

    # Validate against source
    validation = VALIDATE_QUOTE(evidence_text, source_document)
    if not validation.is_valid:
        return SELECT_EVIDENCE(source_document, argument_context)

    return Evidence(text=evidence_text, source_ids=selected_ids)
\end{verbatim}
...
\textbf{Case-Level Full Validation Rate (CFVR)} measures percentage of cases where ALL cited evidence achieves exact or partial match:

\begin{table}[h]
\centering
\begin{tabular}{lcc}
\toprule
\textbf{Source} & \textbf{Evidence Score} & \textbf{CFVR} \\
\midrule
FOAM System & 78.3 & 76.2\% \\
Human Expert & 51.4 & 8.7\% \\
Zero-Shot AI & 20.5 & 0.0\% \\
\bottomrule
\end{tabular}
